( function () {
	...
	const perPage = qpmAdmin.perPage || 10;
	const batchSize = qpmAdmin.batchSize || 10;
	const defaultImage = qpmAdmin.defaultImage || '';

	wp.apiFetch.use( wp.apiFetch.createNonceMiddleware( qpmAdmin.restNonce ) );

	const els = {
		app: document.getElementById( 'qpm-app' ),
		...
		filterBrand: document.getElementById( 'qpm-filter-brand' ),
		filterPostStatus: document.getElementById( 'qpm-filter-status' ),
		clearFilters: document.getElementById( 'qpm-clear-filters' ),
		list: document.getElementById( 'qpm-list' ),
		sentinel: document.getElementById( 'qpm-scroll-sentinel' ),
		saveBtn: document.getElementById( 'qpm-save' ),
		selectAll: document.getElementById( 'qpm-select-all' ),
		bulkEditBtn: document.getElementById( 'qpm-bulk-edit' ),
		selectedCount: document.getElementById( 'qpm-selected-count' ),
		notice: document.getElementById( 'qpm-notice' ),
		overlay: document.getElementById( 'qpm-overlay' ),
		bulkOverlay: document.getElementById( 'qpm-bulk-overlay' ),
		bulkCancel: document.getElementById( 'qpm-bulk-cancel' ),
		bulkApply: document.getElementById( 'qpm-bulk-apply' ),
		progressBar: document.getElementById( 'qpm-progress-bar' ),
		progressText: document.getElementById( 'qpm-progress-text' ),
	};
	...
	function apiPath( query ) {
		return 'qpm/v1/products?' + new URLSearchParams( query ).toString();
	}

	function apiPathSelectable( query ) {
		return 'qpm/v1/products/selectable?' + new URLSearchParams( query ).toString();
	}
	...
	function syncSelectionToDom() {
		...
		els.list.querySelectorAll( '.qpm-card--editable' ).forEach( function ( card ) {
			const id = parseInt( card.getAttribute( 'data-id' ), 10 );
			const cb = card.querySelector( '.qpm-select-row' );
			if ( cb ) {
	...
	function getFilters() {
		...
		if ( filters.brand && qpmAdmin.brandTaxonomy ) {
			filters.brand_taxonomy = qpmAdmin.brandTaxonomy;
		}
	...
	function lockPage( locked ) {
		if ( els.app ) {
			els.app.classList.toggle( 'qpm-app--locked', locked );
		}
		document.body.classList.toggle( 'qpm-body-locked', locked );
	}
	...
	function updateSaveState() {
		const dirty = els.list.querySelectorAll( '.qpm-card--dirty' );
	...
	function getBulkTargets() {
		...
		ids.forEach( function ( id ) {
			const card = els.list.querySelector( '.qpm-card[data-id="' + id + '"]' );
	...
	function fieldInline( label, controlHtml, extraClass ) {
		let cls = 'qpm-field-block qpm-field-block--inline';
		if ( extraClass ) {
			cls += ' ' + extraClass;
		}
		if ( ! label ) {
			cls += ' qpm-field-block--no-label';
		}
		const labelHtml = label
			? '<span class="qpm-field-block__label">' + escAttr( label ) + '</span>'
			: '';
		return (
			'<div class="' +
			cls +
			'">' +
			labelHtml +
			'<div class="qpm-field-block__control">' +
			controlHtml +
			'</div></div>'
		);
	}
	...
	function cardClass( item ) {
		...
		if ( item.readonly ) {
			...
			cls += ' qpm-card--editable';
		}
		if ( item.row_type === 'variation' ) {
			cls += ' qpm-card--variation';
		}
		return cls;
	}
	...
	function buildStatusBadge( item ) {
		...
		return (
			'<span class="qpm-card__status qpm-card__status--' +
			escAttr( slug ) +
			'">' +
	...
	function buildProductInfo( item ) {
		const title = item.edit_url
			? '<a href="' + escAttr( item.edit_url ) + '" class="qpm-card__title">' + escAttr( item.title ) + '</a>'
			: '<span class="qpm-card__title-text">' + escAttr( item.title ) + '</span>';
		return (
			'<div class="qpm-card__product">' +
			thumbHtml( item ) +
			'<div class="qpm-card__meta">' +
			'<div class="qpm-card__tags">' +
			'<span class="qpm-card__id">#' +
			item.id +
			'</span>' +
			...
			( item.row_type === 'parent'
				? '<span class="qpm-card__badge">' + escAttr( item.product_type ) + '</span>'
				: '' ) +
			( item.sku ? '<span class="qpm-card__sku">SKU: ' + escAttr( item.sku ) + '</span>' : '' ) +
			'</div></div>'
		);
	}
	...
	function buildParentCard( item ) {
		return (
			...
			item.id +
			'" data-readonly="1">' +
			'<div class="qpm-card__row">' +
			buildProductInfo( item ) +
			'<span class="qpm-readonly-note">—</span>' +
			'</div></article>'
		);
	}
	...
	function buildEditableCard( item ) {
		...
		const stockInput =
			'<input type="number" class="qpm-mdc-input qpm-field-stock" min="0" step="1" value="' +
			escAttr( stockQty ) +
			'" data-original="' +
			escAttr( stockQty ) +
			'"' +
			...
			' />';
		const manageInput =
			'<input type="checkbox" class="qpm-field-manage-stock"' +
			manageChecked +
			' data-original="' +
			( item.manage_stock ? '1' : '0' ) +
			'" />';
		const regularInput =
			'<input type="text" class="qpm-mdc-input qpm-field-regular" inputmode="decimal" value="' +
			escAttr( formatPrice( item.regular_price ) ) +
			'" data-original="' +
			escAttr( formatPrice( item.regular_price ) ) +
			'" />';
		const saleInput =
			'<input type="text" class="qpm-mdc-input qpm-field-sale" inputmode="decimal" value="' +
			escAttr( formatPrice( item.sale_price ) ) +
			'" data-original="' +
			escAttr( formatPrice( item.sale_price ) ) +
			'" />';
		const scheduleInput =
			'<div class="qpm-schedule' +
			scheduleHidden +
			'"><input type="date" class="qpm-mdc-input qpm-field-sale-from" value="' +
			escAttr( saleFrom ) +
			'" data-original="' +
			escAttr( saleFrom ) +
			'" /><input type="date" class="qpm-mdc-input qpm-field-sale-to" value="' +
			escAttr( saleTo ) +
			'" data-original="' +
			escAttr( saleTo ) +
			'" /></div>';
		...
		return (
			...
			item.id +
			'" data-readonly="0">' +
			'<div class="qpm-card__select"><input type="checkbox" class="qpm-select-row" aria-label="Select" /></div>' +
			'<div class="qpm-card__row">' +
			buildProductInfo( item ) +
			fieldInline( i18n.manageStock, manageInput, 'qpm-field-block--manage' ) +
			fieldInline( i18n.quantity || 'Quantity', stockInput, 'qpm-field-block--stock' ) +
			fieldInline( i18n.regularPrice || 'Regular', regularInput, 'qpm-field-block--regular' ) +
			fieldInline( i18n.salePrice || 'Sale', saleInput, 'qpm-field-block--sale' ) +
			fieldInline( '', scheduleInput, 'qpm-field-block--schedule' ) +
			'</div></article>'
		);
	}
	...
	function setListState( message ) {
		els.list.innerHTML = '<div class="qpm-list__state">' + escAttr( message ) + '</div>';
	}

	function markDirty( card ) {
		card.classList.add( 'qpm-card--dirty' );
		updateSaveState();
	}
	...
	function checkCardDirty( card ) {
		...
		if ( dirty ) {
			...
			card.classList.remove( 'qpm-card--dirty' );
			updateSaveState();
		}
	}

	function toggleSchedule( card ) {
		const saleInput = card.querySelector( '.qpm-field-sale' );
		const schedule = card.querySelector( '.qpm-schedule' );
		if ( saleInput && schedule ) {
			schedule.classList.toggle( 'qpm-schedule--hidden', saleInput.value.trim() === '' );
		}
	}

	function bindCardEvents( card ) {
		if ( card.dataset.qpmBound === '1' ) {
			return;
		}
		card.dataset.qpmBound = '1';

		if ( card.dataset.readonly === '0' ) {
			card.addEventListener( 'input', function ( e ) {
				const t = e.target;
				if ( t.classList.contains( 'qpm-field-manage-stock' ) ) {
					const qty = card.querySelector( '.qpm-field-stock' );
					if ( qty ) {
						qty.disabled = ! t.checked;
					}
				}
				if ( t.classList.contains( 'qpm-field-sale' ) ) {
					toggleSchedule( card );
				}
				checkCardDirty( card );
			} );
			card.addEventListener( 'change', function ( e ) {
				if ( e.target.classList.contains( 'qpm-select-row' ) ) {
	...
	function loadProducts( reset ) {
		...
		wp.apiFetch( { path: apiPath( query ) } )
			.then( function ( data ) {
				...
				els.list.querySelectorAll( '.qpm-card' ).forEach( bindCardEvents );
				syncSelectionToDom();
				updateSelectionUI();
				updateSaveState();
			} )
	...
	function collectChangesFromCard( card ) {
		const id = parseInt( card.getAttribute( 'data-id' ), 10 );
		const change = { id: id };
		const sku = card.querySelector( '.qpm-field-sku' );
		if ( sku && sku.value !== sku.getAttribute( 'data-original' ) ) {
			change.sku = sku.value;
		}
		const manage = card.querySelector( '.qpm-field-manage-stock' );
		if ( manage ) {
	...
	function collectAllChanges() {
		const changes = [];
		els.list.querySelectorAll( '.qpm-card--dirty' ).forEach( function ( card ) {
	...
	function applyBulkToCard( card, rules ) {
		if ( rules.stock && rules.stock.enabled ) {
			const el = card.querySelector( '.qpm-field-stock' );
			if ( el ) {
				const next = adjustStock( el.value, rules.stock.mode, rules.stock.direction, rules.stock.value );
				setFieldValue( card, '.qpm-field-stock', next );
			}
		}
		if ( rules.manage_stock && rules.manage_stock.enabled ) {
			setFieldValue( card, '.qpm-field-manage-stock', rules.manage_stock.value );
			const qty = card.querySelector( '.qpm-field-stock' );
			if ( qty ) {
		...
		if ( rules.regular_price && rules.regular_price.enabled ) {
			const el = card.querySelector( '.qpm-field-regular' );
			if ( el ) {
				const next = adjustPrice( el.value, rules.regular_price.mode, rules.regular_price.direction, rules.regular_price.value );
				setFieldValue( card, '.qpm-field-regular', next );
			}
		}
		if ( rules.sale_price && rules.sale_price.enabled ) {
			const el = card.querySelector( '.qpm-field-sale' );
			if ( el ) {
				const next = adjustPrice( el.value, rules.sale_price.mode, rules.sale_price.direction, rules.sale_price.value );
				setFieldValue( card, '.qpm-field-sale', next );
				toggleSchedule( card );
			}
		}
		if ( rules.sale_from && rules.sale_from.enabled ) {
			setFieldValue( card, '.qpm-field-sale-from', rules.sale_from.value );
			const sched = card.querySelector( '.qpm-schedule' );
			if ( sched ) {
				sched.classList.remove( 'qpm-schedule--hidden' );
			}
		}
		if ( rules.sale_to && rules.sale_to.enabled ) {
			setFieldValue( card, '.qpm-field-sale-to', rules.sale_to.value );
		}
		checkCardDirty( card );
	}
	...
	function getBulkRules() {
		...
		document.querySelectorAll( '.qpm-bulk-enable' ).forEach( function ( sw ) {
			const field = sw.dataset.field;
			if ( ! sw.checked ) {
				return;
			}
			any = true;
			const row = sw.closest( '.qpm-bulk-row' );

			if ( field === 'regular_price' || field === 'sale_price' ) {
				rules[ field ] = {
					enabled: true,
					mode: row.querySelector( '.qpm-bulk-mode' ).value,
					direction: row.querySelector( '.qpm-bulk-direction' ).value,
					value: row.querySelector( '.qpm-bulk-value' ).value,
				};
			} else if ( field === 'stock' ) {
				rules.stock = {
					enabled: true,
				...
				rules.manage_stock = {
					enabled: true,
					value: row.querySelector( '.qpm-bulk-manage-value' ).value,
				};
			} else if ( field === 'sale_from' || field === 'sale_to' ) {
				rules[ field ] = {
					enabled: true,
					value: row.querySelector( '.qpm-bulk-date' ).value,
				};
			}
		} );
		...
		function runNext( index ) {
			if ( index >= batches.length ) {
				...
				allFailed.forEach( function ( f ) {
					const card = els.list.querySelector( '.qpm-card[data-id="' + f.id + '"]' );
					if ( card ) {
						markDirty( card );
					}
				} );
	...
	function confirmBulkEdit() {
		...
		targets.forEach( function ( target ) {
			if ( target.card ) {
				...
				if ( target.card.classList.contains( 'qpm-card--dirty' ) ) {
	...
	function initBulkForm() {
		document.querySelectorAll( '.qpm-bulk-enable' ).forEach( function ( sw ) {
			sw.addEventListener( 'change', function () {
				const row = sw.closest( '.qpm-bulk-row' );
				const controls = row.querySelector( '.qpm-bulk-row__controls' );
				if ( controls ) {
